...
class Client_decline_view extends CI_Controller {
	...
	{
		$data['prospective_member'] = $this->Create_prospective_member->tampil_data_decline()->result();
		$data['akses'] = $this->M_akses_staff->tampil_data()->result();
		$data['content'] = 'staff/client_decline_view';
        ...
        if ( $id)
            {
                    ...
                    if ($id == true)
                        {   
                            ...
                            $this->Create_prospective_member->update($status,$id);
							 redirect('admin/member_decline_view');
							 $msg = '<html><head>
								</head><body onload="launch_toast()">';
							 $msg .= '</body></html>';
							 $this->session->set_flashdata("notif",$msg);
             ...
             { 
				 ...
				 //echo $id;
                 $this->Create_prospective_member->update($status,$id);
				 redirect('staff/Client_decline_view');
             }
    }
